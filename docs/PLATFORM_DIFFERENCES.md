...
| **Menu bar / shortcuts** | Bottom nav + drawer | Browser menu (none) | `MenuBar` widget (File/Edit/View/Help) + `CallbackShortcuts` for `Ctrl+N` (new chat), `Ctrl+F` (search), `Ctrl+,` (settings) — added in `home_view.dart` Windows branch |
| **Back navigation** | System back → in-app stack (GetX), not app-exit unless at root | Browser back → GetX stack | `window_manager` close guard — back at root prompts close, not immediate exit |
...
| **Installer** | `.apk` / `.aab` via `flutter build apk --release` + `android/key.properties` | `flutter build web` → static `build/web` deploy | `flutter build windows` → `build/windows/runner/Release/cubiclm.exe` + `msix`/`inno` installer (doc step, debug config only — no prod keystore committed per §8.6) |
| **Push / boot persistence** | `flutter_background_service` + `BootReceiver` (`RECEIVE_BOOT_COMPLETED`) | No boot (browser) | No boot (desktop) |
| **Speech-to-text** | `speech_to_text` (Android `RECORD_AUDIO` permission, contextual) | `speech_to_text` Web Speech API (permission-gated) | `speech_to_text` Windows stub (not supported, UI hides mic) |
...
- The business logic (`lib/services/inference_service.dart`) is already decoupled — `if (kIsWeb || defaultTargetPlatform==windows) return 'ERROR: ... Use Cloud'` — so adding a Windows native lib later is a single `local_plugins/llama_flutter_windows` + `pubspec` change, no `shared/` duplication.
