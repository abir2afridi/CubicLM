...
<h1 align="center">CubicLM</h1>

<p align="center">
  <b>📱⚡ Cross-platform AI chat with on-device inference & multi-provider cloud support</b>
</p>

<p align="center">
  <a href="https://github.com/abir2afridi/CubicLM/releases/tag/v1.12.0">
    <img src="https://img.shields.io/github/v/release/abir2afridi/CubicLM?label=release&style=for-the-badge&color=FF4D00" alt="Release">
  </a>
  <a href="https://github.com/abir2afridi/CubicLM/releases/tag/v1.12.0">
    <img src="https://img.shields.io/badge/platform-Android%20%7C%20Windows-blue?style=for-the-badge" alt="Platform">
  </a>
  <a href="https://cubiclm.vercel.app">
    <img src="https://img.shields.io/badge/website-cubiclm.vercel.app-FF4D00?style=for-the-badge" alt="Website">
  </a>
</p>
...
| :--- | :--- | ---: | :--- |
| 🤖 **Android** (arm64) | `cubiclm-v1.12.0-arm64-v8a.apk` | 62 MB | [**GitHub Release**](https://github.com/abir2afridi/CubicLM/releases/download/v1.12.0/cubiclm-v1.12.0-arm64-v8a.apk) |
| 🤖 **Android** (arm32) | `cubiclm-v1.12.0-armeabi-v7a.apk` | 16 MB | [**GitHub Release**](https://github.com/abir2afridi/CubicLM/releases/download/v1.12.0/cubiclm-v1.12.0-armeabi-v7a.apk) |
| 🤖 **Android** (x86_64) | `cubiclm-v1.12.0-x86_64.apk` | 28 MB | [**GitHub Release**](https://github.com/abir2afridi/CubicLM/releases/download/v1.12.0/cubiclm-v1.12.0-x86_64.apk) |
| 🪟 **Windows** (x64) | `cubiclm-v1.12.0-windows-x64.zip` | 17 MB | [**GitHub Release**](https://github.com/abir2afridi/CubicLM/releases/download/v1.12.0/cubiclm-v1.12.0-windows-x64.zip) — unzip & run `cubiclm.exe` (WebView2 required) |
| 🔒 **Checksums** | `checksums.sha256` | — | [**GitHub Release**](https://github.com/abir2afridi/CubicLM/releases/download/v1.12.0/checksums.sha256) |

> **🌐 Website:** [cubiclm.vercel.app](https://cubiclm.vercel.app) — landing + direct APK / Windows downloads
> **📝 Full notes:** [CHANGELOG.md](CHANGELOG.md) · [Release page](https://github.com/abir2afridi/CubicLM/releases/tag/v1.12.0)
> **ℹ️ Web** (`dart:ffi`) is tracked for a future release — **Android + Windows** are the supported targets for `v1.12.0`.
...
- [✨ Features](#-features)
  ...
  - [🚀 Startup, Onboarding & Splash](#-startup-onboarding--splash)
  - [🌐 Web Access](#-web-access-independent-chat)
  - [🧠 Chat Context Memory](#-chat-context-memory-follow-ups-dont-get-lost)
  - [☁️ Cloud AI Providers](#️-cloud-ai-providers)
  - [🧩 Skills](#-skills--offline-instruction-extensions)
  - [🔌 Custom MCP Server](#-custom-mcp-server--single-remote-connection-no-marketplace)
  - [🔌 Built-in OpenAI-Compatible API Server](#-built-in-openai-compatible-api-server)
  - [🧩 Additional](#-additional)
  - [🌐 CubicWeb Builder](#-cubicweb-builder-toolkit--v1120)
  - [🖥️ Universal Multi-Platform](#️-universal-multi-platform--one-codebase-three-shells)
...
| 👁️ **Vision models** | Qwen2-VL-2B, Gemma 4 E2B/E4B for image understanding |
| 🎬 **Streaming token generation** | real-time tokens-per-second display — long answers stream as plain text (full markdown on completion) with adaptive flush (150/300ms) so the chat never janks |
| 🛡️ **GPU crash recovery** | automatic CPU fallback if GPU backend fails |
...
- Cloud requests sent **without an output cap**, so large models write full detailed answers instead of stopping mid-response
- An inline **(i) info dialog** explains exactly what it does

</td>
<td width="50%" valign="top">

**🎚️ Manual mode** — flip Auto off for direct control with extended ranges:
...
| 🛡️ **RAM guard** *(new)* | `DeviceInfoService.canAllocateContextSize()` (`fileBytes + KV~2.5KB×context` vs 80% available RAM); `SettingsController.setContextSize` clamps to `maxSafeContextSize` with snackbar, `ModelController._confirmModelLoadSafety` includes KV estimate before showing *Restart recommended* |
| 🎨 **Image parameters** | Sampling steps and synthesis resolution for image generation (Auto mode scales by available RAM) |
...
- **🔄 Auto-load last model** — `App Settings → STARTUP` switch (`AppConstants.keyAutoLoadLastModel`):
  - **ON**: after chat UI is idle (splash gone + `DeviceInfo` ready + `1200ms`), checks `isModelLoaded` resident, `90s` crash-loop guard, `80%` RAM guard, then `loadModel().timeout(90s)` off the UI frame
...
<details>
<summary><b>🔧 Architecture & Data Model</b></summary>
...
| ✍️ **Creative Writer** | stories/poems/scripts |
...
- GitHub browse is a flat list (no search/categories) with per-item Import; URL import shows preview (frontmatter-parsed name/description) before save
...
#### 🔨 Implementation
- **SDK** — `dart_mcp ^0.5.2` (labs.dart.dev) + `mcp_client` fallback checked; only remote transport is exposed. Hand-rolled JSON-RPC avoided where the SDK fits; Flutter ergonomics fallback is HTTP/SSE via `dio`
...
- `OpenAICompatibleProvider.sendMessage/streamMessage` detects `tool_calls`, calls `McpRegistryService.callTool`, round-trips `tool` result via second request, then returns final answer (streaming buffers tool deltas and re-emits final answer chunked)
...
- Live status dot + banner, and exposed-tools list (name + description) before enabling
...
| :--- | :--- |
| **Explore** | Two tabs — **Model Hub** (Local / Online / Skills / MCP scopes) and **Toolkit** (Battle Arena + Slide Maker + CubicWeb Builder widget cards with descriptions) |
| **Nodes** | Two tabs — **Node** (local API server) and **Config** (diagnostics, hardware capabilities, inference mode, system prompt, Skills, Custom MCP Server, local model & imaging parameters) |
| **App Settings** | Theme mode, typography scale, **Thinking Orbs** (custom animation per context: chatting / image generation / analyzing — each set to **Random** or any of the 9 states with live preview), **Language** (15 languages including Bangla, Hindi, Arabic, Chinese, Spanish, French, Japanese, Korean, Portuguese, German, Turkish, Indonesian, Russian, Urdu — instant switch, Hive-persisted), **Startup → Auto-load last model**, and app info (tap to open **About** page with feature highlights, tech stack, and GitHub link) |
...
- **Background model download** with foreground service — downloads keep running when the app is closed or swiped away; notification with Pause/Cancel actions; HTTP Range resume picks up at the exact byte offset after pause or app restart (`START_STICKY`)
  - **Resume race + atomicity fixed** *(new)* — `download_service:68` atomic `.tmp→rename` for `paused_downloads.json`, `download_native:427` `validateDownloadedFile()` (GGUF `GGUF` / litertlm `LITERTLM` / safetensors `{` + 1% size), `reconcileActiveDownloads` prefers native and cleans stale completed before restore, `model_controller:199` async header check for imported files
...
| Feature | Details |
| :--- | :--- |
| 🎬 **Live streaming builds** | files appear in the explorer as the AI writes (writing badges, live highlighted editor); static preview reloads live; cancel rolls back to checkpoint |
| 🗺️ **Plan mode & checkpoints** | checkpoints/undo, diff view, responsive viewports (mobile/tablet/desktop), screenshot-to-code, keyboard shortcuts, 6 project templates |
| 🖥️ **Real local runtime** | project detection (static/Vite/Next/Node), source validation, dev-server manager (install, free-port pick, URL parse, health check, crash watcher, restart), real shell terminal |
| 🚀 **Deploy + share** | one-click Vercel/Netlify deploy, share links, GitHub export, component library, extended-thinking + web-search toggles, browser auto-test |
| ⚛️ **Next.js pipeline** | atomic writes, byte-for-byte integrity gates, framework adapters, `npm run build` validation, external-browser preview |
| 🧑‍💻 **Developer terminal + CLI manager** | real shell with history/autocomplete/stdin attach; one-tap installs for Claude Code, OpenCode, Cline, Kilo; Open/Verify/Update/Repair/Uninstall with persistent registry |
...
| ☁️ **Cloud model tools** | import from `/models` with new-count report, test-all with online/failed dots, auto-hide failed, auto-sync interval (configurable) |
...
| 🏷️ **Category filters** | System, Model, Cloud, Chat, Server, Image |
...
<details>
<summary><b>🗂️ Click to expand full tree</b></summary>
...
| ☕ **Java** | 17 |
| 🔑 **Keystore** | For release builds: configured at `android/key.properties` (see `android/key.properties.example`) |
...
1. 📋 Paste the key
2. ✅ Hit **Verify** (live endpoint check, nothing saved)
3. 💾 **Save Key** unlocks once it shows *Verified — N models found*
...
Refresh any time with the ↻ button on a provider card, or probe every model with the activity (Test all) button.
...
- Loading is guarded — missing or corrupted files are detected before reaching the native engine, and stale download pointers are cleaned up automatically
...
- **Save / Test Connection / Enable-Disable** (with tool-preview dialog) **/ Remove** with live status (disconnected/connecting/connected/error)
...
- app info (View Update center + settings)
...
<p align="center">
  <sub>Built with ❤️ by <a href="https://github.com/abir2afridi">Abir Afridi</a></sub>
</p>
