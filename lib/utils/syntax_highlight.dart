/// Lightweight regex-based syntax highlighter for HTML/CSS/JS.
...
List<SyntaxToken> _highlightJs(String src) {
  final tokens = <SyntaxToken>[];
  final kw = RegExp(
  ...
  final re = RegExp(
    ...
    r"|('(\\.|[^'\\])*')"          // 4: single string
    r'|(`(\\.|[^`])*`)'            // 5: template string
    r'|(\b\d+\.?\d*([eE][+-]?\d+)?\b)' // 6: number
    r'|([a-zA-Z_$][\w$]*\s*:)'     // 7: object key
    r'|([{}();:,.\[\]=+\-<>&|!?])'  // 8: punct
    r'|(\s+)',                       // 9: ws
    caseSensitive: false,
  );
  for (final m in re.allMatches(src)) {
    if (m.group(1) != null) {
      ...
      tokens.add(SyntaxToken(m.group(2)!, SyntaxColors.comment));
    } else if (m.group(3) != null) {
      tokens.add(SyntaxToken(m.group(3)!, SyntaxColors.string));
    } else if (m.group(4) != null) {
      tokens.add(SyntaxToken(m.group(4)!, SyntaxColors.string));
      ...
      tokens.add(SyntaxToken(m.group(6)!, SyntaxColors.number));
    } else {
      // Check if this match is a keyword
...
TextSpan buildHighlightedSpan(List<SyntaxToken> tokens) {
  return TextSpan(
    children: tokens
        .map((t) => TextSpan(
              text: t.text,
              style: TextStyle(color: t.color, fontFamily: 'FiraCode', fontSize: 12, height: 1.5),
            ))
        .toList(),
  );
}
